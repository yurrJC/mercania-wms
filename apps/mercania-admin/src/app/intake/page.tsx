            ...
            {duplicateWarning?.isDuplicate && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                <div className="flex items-center mb-2">
                  ...
                  <h3 className="font-semibold text-yellow-800">Duplicate Book Detected</h3>
                </div>
                <p className="text-yellow-700 mb-3">{duplicateWarning.message}</p>
                <p className="text-yellow-600 text-sm font-medium mb-3">{duplicateWarning.recommendation}</p>
                
                {duplicateWarning.existingItems && duplicateWarning.existingItems.length > 0 && (
                  <div className="bg-yellow-100 rounded-md p-3">
                    <p className="text-sm font-medium text-yellow-800 mb-2">Previous copies:</p>
                    <div className="space-y-1">
                      {duplicateWarning.existingItems.map((item) => (
                        <div key={item.id} className="text-xs text-yellow-700 flex justify-between">
                          <span>ID #{item.id}</span>
                          <span className="font-medium">{item.status}</span>
                          <span>{new Date(item.intakeDate).toLocaleDateString()}</span>
                        </div>
                      ))}
                    </div>
                  </div>
